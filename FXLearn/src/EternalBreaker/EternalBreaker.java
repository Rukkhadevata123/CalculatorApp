...
public class EternalBreaker extends Application {
    ...
    public static final double HELL_SPEED = 8;
    ...
    private Stopwatch stopwatch2; // Create a stopwatch to record the time used in the last round
    private int timeLeft = 3; // Initialize the preparation time to 3 seconds
    private int gameRound = 1; // Initialize the game round to 1
    ...
    public void start(Stage primaryStage) {
        ...
        Button slowSpeedButton = new Button("Slow(" + SLOW_SPEED + ")");
        Button normalSpeedButton = new Button("Normal(" + NORMAL_SPEED + ")");
        Button fastSpeedButton = new Button("Fast(" + FAST_SPEED + ")");
        Label lengthLabel = new Label("Paddle Length:");
        Button longLengthButton = new Button("Long(" + LONG_LENGTH + ")");
        Button normalLengthButton = new Button("Normal(" + NORMAL_LENGTH + ")");
        Button shortLengthButton = new Button("Short(" + SHORT_LENGTH + ")");
        ...
        paddle = new Rectangle((double) WIDTH / 2 - NORMAL_LENGTH / 2, PADDLE_Y, NORMAL_LENGTH, PADDLE_HEIGHT);
        ...
        slowSpeedButton.setOnAction(e -> {
            if (!isRandomMode && dx != HELL_SPEED || !isGameRunning) { // Allow to switch to slow speed when
                // game is not running or
                // in hell mode
                dx = dy = SLOW_SPEED;
                isRandomMode = false;
                speedLabel2.setText("Speed: " + "Slow(" + SLOW_SPEED + ")");
            }
        });

        normalSpeedButton.setOnAction(e -> {
            if (!isRandomMode && dx != HELL_SPEED || !isGameRunning) { // Allow to switch to normal speed when
                // game is not running or
                // in hell mode
                dx = dy = NORMAL_SPEED;
                isRandomMode = false;
                speedLabel2.setText("Speed: " + "Normal(" + NORMAL_SPEED + ")");
            }
        });

        fastSpeedButton.setOnAction(e -> {
            if (!isRandomMode && dx != HELL_SPEED || !isGameRunning) { // Allow to switch to fast speed when
                // game is not running or
                // in hell mode
                dx = dy = FAST_SPEED;
                isRandomMode = false;
                speedLabel2.setText("Speed: " + "Fast(" + FAST_SPEED + ")");
            }
        });

        longLengthButton.setOnAction(e -> {
            if (!isRandomMode && dx != HELL_SPEED || !isGameRunning) { // Allow to switch to long length when
                // game is not running or
                // in hell mode
                paddle.setWidth(LONG_LENGTH);
                paddle.setVisible(true);
                isRandomMode = false;
                lengthLabel2.setText("Length: " + "Long(" + LONG_LENGTH + ")");
            }
        });

        normalLengthButton.setOnAction(e -> {
            if (!isRandomMode && dx != HELL_SPEED || !isGameRunning) { // Allow to switch to normal length
                // when game is not running
                // or in hell mode
                paddle.setWidth(NORMAL_LENGTH);
                paddle.setVisible(true);
                isRandomMode = false;
                lengthLabel2.setText("Length: " + "Normal(" + NORMAL_LENGTH + ")");
            }
        });

        shortLengthButton.setOnAction(e -> {
            if (!isRandomMode && dx != HELL_SPEED || !isGameRunning) { // Allow to switch to short length when
                // game is not running or
                // in hell mode
                paddle.setWidth(SHORT_LENGTH);
                paddle.setVisible(true);
                isRandomMode = false;
                lengthLabel2.setText("Length: " + "Short(" + SHORT_LENGTH + ")");
            }
        });
        ...
        hellMButton.setOnAction(e -> {
            if (!isGameRunning) {
                ...
                speedLabel2.setText("Speed: " + "HELL(" + HELL_SPEED + ")");
                lengthLabel2.setText("Length: " + "TRANSPARENT(" + NORMAL_LENGTH + ")");
            }
        });
        ...
        scoreLabel = createLabel("Score: 0", 10, 100, true);
        roundLabel = createLabel("Round: 1", 10, 120, true);
        lastRoundTimeLabel = createLabel("Time used in the last round: 0.0 seconds", 10, 140, true);
        timeUsedLabel = createLabel("Time used in all rounds: 0.0 seconds", 10, 160, true);
        speedLabel2 = createLabel("Speed: " + "Normal(" + NORMAL_LENGTH + ")", 10, 180, true);
        lengthLabel2 = createLabel("Length: " + "Normal(" + NORMAL_LENGTH + ")", 10, 200, true);
        messageLabel = createLabel("Get ready!", (double) (WIDTH - 100) / 2, (double) HEIGHT / 2 - 100, false);
...
//        Timeline updater = new Timeline(new KeyFrame(Duration.millis(100), e -> label.setText(text)));
//        updater.setCycleCount(Timeline.INDEFINITE);
//        updater.play();
//    }

    private void updateScoreLabel() {
        Timeline scoreUpdater = new Timeline(new KeyFrame(Duration.millis(100), e -> scoreLabel.setText("Score: " + score)));
        scoreUpdater.setCycleCount(Timeline.INDEFINITE);
        scoreUpdater.play();
    }

    private void updateRoundLabel() {
        Timeline roundUpdater = new Timeline(new KeyFrame(Duration.millis(100), e -> roundLabel.setText("Round: " + gameRound)));
        roundUpdater.setCycleCount(Timeline.INDEFINITE);
        roundUpdater.play();
    }

    private void updateTimeUsedLabel() {
        Timeline timeUsedUpdater = new Timeline(new KeyFrame(Duration.millis(100), e -> timeUsedLabel.setText("Time used in all rounds: " + timeUsed + " seconds")));
        timeUsedUpdater.setCycleCount(Timeline.INDEFINITE);
        timeUsedUpdater.play();
    }

    private void updateTimeUsedLabel2() {
        Timeline timeUsedUpdater2 = new Timeline(new KeyFrame(Duration.millis(100), e -> lastRoundTimeLabel.setText("Time used in the last round: " + timeUsed2 + " seconds")));
    ...
    private void updateScore() {
        ...
        if (dx == HELL_SPEED && dy == HELL_SPEED) {
            score += 6;
        }
        if (!paddle.isVisible()) {
            score += 6;
        ...
        if (dx != HELL_SPEED && dy != HELL_SPEED) {
            ...
            switch ((int) paddle.getWidth()) {
                case (int) LONG_LENGTH:
                    score += 1;
                    break;
                case (int) NORMAL_LENGTH:
                    score += 2;
                    break;
                case (int) SHORT_LENGTH:
                    score += 3;
                    break;
            }
        }
    ...
    private void resetGame() {
        ...
        paddle.setX((double) WIDTH / 2 - NORMAL_LENGTH / 2);
    ...
    private void moveBall() throws InterruptedException {
        ...
        if (ball.getCenterX() < BALL_RADIUS + 1 || ball.getCenterX() > WIDTH - BALL_RADIUS - 1) {
            ...
            if (isRandomMode) {
                angle += random.nextDouble() * 0.3 - 0.15; // Random number between -0.15 and 0.15
            }
        }
        ...
        if (ball.getCenterY() > HEIGHT - BALL_RADIUS) {
            ...
            paddle.setVisible(dx != HELL_SPEED || !lengthText.equals("Length: " + "TRANSPARENT(" + NORMAL_LENGTH + ")"));
        ...
        if (ball.intersects(paddle.getBoundsInLocal())) {
            ...
            if (hitFactor == 0) {
                ...
                angle = hitFactor < 0 ? Math.PI - Math.max(hitFactor * -Math.PI / 2, Math.PI / 6) : Math.max(hitFactor * Math.PI / 2, Math.PI / 6);
            }
